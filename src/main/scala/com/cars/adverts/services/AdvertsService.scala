...
import com.cars.adverts.dao.{CarAdvert, CarAdvertsRepository, FuelTypeRepository, VehicleConditionRepository}
...
class AdvertsService @Inject()(carAdvertsRepository: CarAdvertsRepository,
   ...
   private def createOrUpdate(carAdvert: CarAdvert) = {
      val id  = carAdvert.id
      id match {
      ...
      fuelTypeRepository.getByFuelTypeDesc(advertisement.fuelType).flatMap {
         case Some(fuelType) => vehicleConditionRepository.getByCondition(advertisement.condition).flatMap {
            case Some(vehicleCondition) => {
               val carAdvert = CarAdvert(advertisement.id,fuelType.id,advertisement.title,advertisement.price,vehicleCondition.id,null,null)
